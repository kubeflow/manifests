...
func writeMinioOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/pipeline/minio/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: minio
...
`)
	th.writeK("/manifests/pipeline/minio/overlays/istio", `
...
func TestMinioOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pipeline/minio/overlays/istio")
	writeMinioOverlaysIstio(th)
	...
	targetPath := "../pipeline/minio/overlays/istio"
	fsys := fs.MakeRealFS()
