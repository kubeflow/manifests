...
func writeCiPipelineRunOverlaysCpuNodeSelector(th *KustTestHarness) {
	th.writeF("/manifests/ci/ci-pipeline-run/overlays/cpu-node-selector/pipeline-run.yaml", `
...
spec:
  podTemplate:
    nodeSelector:
      cpu: "$(cpuType)"
`)
	th.writeF("/manifests/ci/ci-pipeline-run/overlays/cpu-node-selector/params.yaml", `
varReference:
- path: spec/podTemplate/nodeSelector/cpu
  kind: PipelineRun
`)
	th.writeF("/manifests/ci/ci-pipeline-run/overlays/cpu-node-selector/params.env", `
generateName=
cpuType=cascadelake
`)
	th.writeK("/manifests/ci/ci-pipeline-run/overlays/cpu-node-selector", `
...
func TestCiPipelineRunOverlaysCpuNodeSelector(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/ci/ci-pipeline-run/overlays/cpu-node-selector")
	writeCiPipelineRunOverlaysCpuNodeSelector(th)
	...
	targetPath := "../ci/ci-pipeline-run/overlays/cpu-node-selector"
	fsys := fs.MakeRealFS()
