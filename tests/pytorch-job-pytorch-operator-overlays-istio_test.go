...
func writePytorchOperatorOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/pytorch-job/pytorch-operator/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
...
`)
	th.writeK("/manifests/pytorch-job/pytorch-operator/overlays/istio", `
...
func TestPytorchOperatorOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pytorch-job/pytorch-operator/overlays/istio")
	writePytorchOperatorOverlaysIstio(th)
	...
	targetPath := "../pytorch-job/pytorch-operator/overlays/istio"
	fsys := fs.MakeRealFS()
