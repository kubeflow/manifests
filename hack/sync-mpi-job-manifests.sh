...
SRC_DIR=${SRC_DIR:=/tmp/kubeflow-mpi-operator}
BRANCH=${BRANCH:=sync-kubeflow-mpi-operator-manifests-${COMMIT?}}
...
echo "Copying mpi-operator manifests..."
DST_DIR=$MANIFESTS_DIR/apps/mpi-job/upstream
rm -r $DST_DIR
...
cd $MANIFESTS_DIR
git add apps
git commit -m "Update kubeflow/mpi-operator manifests from ${COMMIT}"
