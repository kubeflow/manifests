...
func writeCiCreateClusterTaskRunOverlaysApplication(th *KustTestHarness) {
	th.writeF("/manifests/ci/ci-create-cluster-task-run/overlays/application/application.yaml", `
...
spec:
  componentKinds:
    - group: app.k8s.io
      kind: Application
  descriptor: 
    type: ci-create-cluster
    version: v1beta1
    description: application that creates a cluster in gcp
    maintainers:
    - name: Kam Kasravi
  ...
  addOwnerRef: true
`)
	th.writeF("/manifests/ci/ci-create-cluster-task-run/overlays/application/params.yaml", `
varReference:
- path: metadata/name
  kind: Application
`)
	th.writeF("/manifests/ci/ci-create-cluster-task-run/overlays/application/params.env", `
generateName=
`)
	th.writeK("/manifests/ci/ci-create-cluster-task-run/overlays/application", `
...
- application.yaml
configMapGenerator:
- name: ci-create-cluster-app-parameters
  env: params.env
vars:
- name: generateName
  objref:
...
- params.yaml
commonLabels:
  app.kubernetes.io/name: ci-create-cluster
...
func TestCiCreateClusterTaskRunOverlaysApplication(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/ci/ci-create-cluster-task-run/overlays/application")
	writeCiCreateClusterTaskRunOverlaysApplication(th)
	...
	targetPath := "../ci/ci-create-cluster-task-run/overlays/application"
	fsys := fs.MakeRealFS()
