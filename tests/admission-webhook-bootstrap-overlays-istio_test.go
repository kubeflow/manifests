...
func writeBootstrapOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/admission-webhook/bootstrap/overlays/istio/stateful-set.yaml", `
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: stateful-set
...
`)
	th.writeK("/manifests/admission-webhook/bootstrap/overlays/istio", `
...
func TestBootstrapOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/admission-webhook/bootstrap/overlays/istio")
	writeBootstrapOverlaysIstio(th)
	...
	targetPath := "../admission-webhook/bootstrap/overlays/istio"
	fsys := fs.MakeRealFS()
