...
func writeApplicationOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/application/application/overlays/istio/stateful-set.yaml", `
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: stateful-set
...
`)
	th.writeK("/manifests/application/application/overlays/istio", `
...
func TestApplicationOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/application/application/overlays/istio")
	writeApplicationOverlaysIstio(th)
	...
	targetPath := "../application/application/overlays/istio"
	fsys := fs.MakeRealFS()
