...
def get_resource_key(manifest: Dict, component: str = "katib") -> str:
    ...
    # Include namespace whenever it exists to avoid collisions across namespaces.
    if namespace:
...
def get_expected_helm_extras(component: str, scenario: str, namespace: str) -> set:
    """Get expected extra resources in Helm for different components and scenarios."""
    if component == "katib":
        return {
            f'Secret/{namespace}/katib-webhook-cert',  # Webhook certificates
        }
        ...
        if scenario == "base":
            return {f"AuthorizationPolicy/{namespace}/kserve-models-web-app"}
        return set()
    else:
        return set()
...
def compare_manifests(
    ...
    expected_helm_extras = get_expected_helm_extras(component, scenario, namespace)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare manifests generated by Kustomize and Helm"
    )
    parser.add_argument("kustomize_file")
    parser.add_argument("helm_file")
    ...
    kustomize_file = args.kustomize_file
    helm_file = args.helm_file
