...
func writePachydermOverlaysLocal(th *KustTestHarness) {
	th.writeF("/manifests/pachyderm/overlays/local/role.yaml", `
...
- apiGroups:
  ...
  - 'update'
  - 'delete'`)
	th.writeF("/manifests/pachyderm/overlays/local/role-binding.yaml", `
apiVersion: rbac.authorization.k8s.io/v1beta1
kind: RoleBinding
metadata:
  name: pachyderm
...
`)
	th.writeF("/manifests/pachyderm/overlays/local/params.yaml", `
varReference:
- path: subjects/namespace
  kind: RoleBinding
- path: subjects/name
  kind: RoleBinding`)
	th.writeF("/manifests/pachyderm/overlays/local/params.env", `
STORAGE_BACKEND=LOCAL
`)
	th.writeK("/manifests/pachyderm/overlays/local", `
...
func TestPachydermOverlaysLocal(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pachyderm/overlays/local")
	writePachydermOverlaysLocal(th)
	m, err := th.makeKustTarget().MakeCustomizedResMap()
	if err != nil {
		t.Fatalf("Err: %v", err)
	}
	targetPath := "../pachyderm/overlays/local"
	fsys := fs.MakeRealFS()
