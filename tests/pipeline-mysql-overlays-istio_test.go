...
func writeMysqlOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/pipeline/mysql/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: mysql
...
`)
	th.writeK("/manifests/pipeline/mysql/overlays/istio", `
...
func TestMysqlOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pipeline/mysql/overlays/istio")
	writeMysqlOverlaysIstio(th)
	...
	targetPath := "../pipeline/mysql/overlays/istio"
	fsys := fs.MakeRealFS()
