...
func writeScheduledworkflowOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/pipeline/scheduledworkflow/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
...
`)
	th.writeK("/manifests/pipeline/scheduledworkflow/overlays/istio", `
...
func TestScheduledworkflowOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pipeline/scheduledworkflow/overlays/istio")
	writeScheduledworkflowOverlaysIstio(th)
	...
	targetPath := "../pipeline/scheduledworkflow/overlays/istio"
	fsys := fs.MakeRealFS()
