...
func writeTektoncdOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/tektoncd/tektoncd/overlays/istio/virtual-service.yaml", `
  ...
  - match:
    ...
    - destination:
        ...
        port:
          number: 80
`)
	th.writeF("/manifests/tektoncd/tektoncd/overlays/istio/params.yaml", `
varReference:
- path: spec/http/route/destination/host
  kind: VirtualService
`)
	th.writeK("/manifests/tektoncd/tektoncd/overlays/istio", `
...
func TestTektoncdOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/tektoncd/tektoncd/overlays/istio")
	writeTektoncdOverlaysIstio(th)
	m, err := th.makeKustTarget().MakeCustomizedResMap()
	if err != nil {
		t.Fatalf("Err: %v", err)
	}
	targetPath := "../tektoncd/tektoncd/overlays/istio"
	fsys := fs.MakeRealFS()
