...
func writeCloudEndpointsOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/gcp/cloud-endpoints/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
...
`)
	th.writeK("/manifests/gcp/cloud-endpoints/overlays/istio", `
...
func TestCloudEndpointsOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/gcp/cloud-endpoints/overlays/istio")
	writeCloudEndpointsOverlaysIstio(th)
	...
	targetPath := "../gcp/cloud-endpoints/overlays/istio"
	fsys := fs.MakeRealFS()
