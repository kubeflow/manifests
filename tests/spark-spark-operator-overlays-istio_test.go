...
func writeSparkOperatorOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/spark/spark-operator/overlays/istio/deploy.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: sparkoperator
...
`)
	th.writeK("/manifests/spark/spark-operator/overlays/istio", `
...
func TestSparkOperatorOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/spark/spark-operator/overlays/istio")
	writeSparkOperatorOverlaysIstio(th)
	...
	targetPath := "../spark/spark-operator/overlays/istio"
	fsys := fs.MakeRealFS()
