...
func writeWebhookOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/admission-webhook/webhook/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
  name: deployment
...
`)
	th.writeK("/manifests/admission-webhook/webhook/overlays/istio", `
...
func TestWebhookOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/admission-webhook/webhook/overlays/istio")
	writeWebhookOverlaysIstio(th)
	...
	targetPath := "../admission-webhook/webhook/overlays/istio"
	fsys := fs.MakeRealFS()
