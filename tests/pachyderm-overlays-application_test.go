...
func writePachydermOverlaysApplication(th *KustTestHarness) {
	th.writeF("/manifests/pachyderm/overlays/application/application.yaml", `
...
func TestPachydermOverlaysApplication(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pachyderm/overlays/application")
	writePachydermOverlaysApplication(th)
	m, err := th.makeKustTarget().MakeCustomizedResMap()
	if err != nil {
		t.Fatalf("Err: %v", err)
	}
	targetPath := "../pachyderm/overlays/application"
	fsys := fs.MakeRealFS()
