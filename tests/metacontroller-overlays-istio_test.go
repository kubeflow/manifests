...
func writeMetacontrollerOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/metacontroller/overlays/istio/stateful-set.yaml", `
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: metacontroller
...
`)
	th.writeK("/manifests/metacontroller/overlays/istio", `
...
func TestMetacontrollerOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/metacontroller/overlays/istio")
	writeMetacontrollerOverlaysIstio(th)
	...
	targetPath := "../metacontroller/overlays/istio"
	fsys := fs.MakeRealFS()
