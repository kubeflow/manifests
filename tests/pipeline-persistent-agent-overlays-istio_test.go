...
func writePersistentAgentOverlaysIstio(th *KustTestHarness) {
	th.writeF("/manifests/pipeline/persistent-agent/overlays/istio/deployment.yaml", `
apiVersion: apps/v1
kind: Deployment
metadata:
...
`)
	th.writeK("/manifests/pipeline/persistent-agent/overlays/istio", `
...
func TestPersistentAgentOverlaysIstio(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pipeline/persistent-agent/overlays/istio")
	writePersistentAgentOverlaysIstio(th)
	...
	targetPath := "../pipeline/persistent-agent/overlays/istio"
	fsys := fs.MakeRealFS()
