...
func writePachydermOverlaysGcp(th *KustTestHarness) {
	th.writeF("/manifests/pachyderm/overlays/gcp/cluster-role.yaml", `
...
- apiGroups:
  ...
  - 'delete'
`)
	th.writeF("/manifests/pachyderm/overlays/gcp/cluster-role-binding.yaml", `
apiVersion: rbac.authorization.k8s.io/v1
...
`)
	th.writeF("/manifests/pachyderm/overlays/gcp/secrets.env", `
GOOGLE_BUCKET=
GOOGLE_CRED=`)
	th.writeK("/manifests/pachyderm/overlays/gcp", `
...
func TestPachydermOverlaysGcp(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/pachyderm/overlays/gcp")
	writePachydermOverlaysGcp(th)
	...
	targetPath := "../pachyderm/overlays/gcp"
	fsys := fs.MakeRealFS()
