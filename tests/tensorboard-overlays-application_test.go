...
func writeTensorboardOverlaysApplication(th *KustTestHarness) {
	th.writeF("/manifests/tensorboard/overlays/application/application.yaml", `
...
`)
	th.writeF("/manifests/tensorboard/overlays/application/params.yaml", `
varReference:
- path: metadata/name
  kind: Application
...
- path: spec/template/metadata/labels/app.kubernetes.io\/instance
  kind: Deployment
`)
	th.writeF("/manifests/tensorboard/overlays/application/params.env", `
generateName=
`)
	th.writeK("/manifests/tensorboard/overlays/application", `
...
- application.yaml
configMapGenerator:
- name: tensorboard-app-parameters
  env: params.env
vars:
- name: generateName
  objref:
...
func TestTensorboardOverlaysApplication(t *testing.T) {
	th := NewKustTestHarness(t, "/manifests/tensorboard/overlays/application")
	writeTensorboardOverlaysApplication(th)
	...
	targetPath := "../tensorboard/overlays/application"
	fsys := fs.MakeRealFS()
